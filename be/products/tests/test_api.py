...
class ProductAPITestCase(TransactionTestCase):
    ...
    def test_filter_products_by_category(self):
        ...
        # ``ProductListSerializer`` exposes ``category_name`` (not the raw
        # ``category`` FK ID) - so assert on the human-readable label.
        for product in products:
            self.assertEqual(product['category_name'], self.main_category.name)
    ...
    def test_filter_products_by_supplier(self):
        ...
        # ``Product.supplier`` is a FK to ``Supplier`` - the legacy free-text
        # field for the supplier label is ``supplier_name``. Use the legacy
        # field so this test exercises name-based supplier lookup without
        # depending on a real Supplier row.
