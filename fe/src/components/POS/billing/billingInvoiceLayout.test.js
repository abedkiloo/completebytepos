...
describe('billingInvoiceLayout', () => {
  ...
  it('allows the right column to scroll instead of clipping content', () => {
    expect(BILLING_RIGHT_COLUMN_CLASS).toMatch(/app-scroll-region/);
    expect(BILLING_RIGHT_COLUMN_CLASS).toMatch(/max-h-/);
  });

  it('allows the left column to scroll inside the viewport', () => {
    expect(BILLING_LEFT_COLUMN_CLASS).toMatch(/app-scroll-region/);
    expect(BILLING_LEFT_COLUMN_CLASS).toMatch(/min-h-0/);
  });
